...
extension APIClient{
    ...
    static func checkDeliveryLogin(p_lang_no:String, completionHandler : @escaping(Result<RootClass? , Result_Error>) -> Void) {
        ad?.isLoading()
        performSwiftyRequest(route: .getDeliveryStatusTypes(p_lang_no:p_lang_no)) { (jsonData) in
            let json = JSON(jsonData)
            guard let errNo = json["Result"]["ErrNo"].int,
            errNo != 0 else {
                DispatchQueue.main.async {
    ...
    static func getDeliveryStatusTypes(p_lang_no:String, completionHandler : @escaping(Result<RootClass? , Result_Error>) -> Void) {
        ad?.isLoading()
        performSwiftyRequest(route: .getDeliveryStatusTypes(p_lang_no:p_lang_no)) { (jsonData) in
            let json = JSON(jsonData)
            guard let errNo = json["Result"]["ErrNo"].int,
